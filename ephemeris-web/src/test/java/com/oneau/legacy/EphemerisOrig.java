...
class EphemerisOrig {
    ...
    */
    ...
    public EphemerisOrig(String datafile) {
        this.dataFile = datafile;
    }
    public EphemerisOrig() {
    }
	...
	*/
        ...
        System.out.println(format("jultime: %s, ephemeris_dates[1]: %s, INTERVAL_DURATION: %s", jultime, ephemeris_dates[1], interval_duration));
	...
	*/
        ...
        System.out.println(format("interval:%s, interval_start_time: %s, subinterval_duration: %s, subinterval: %s, numbers_to_skip: %s, pointer: %s", interval, interval_start_time, subinterval_duration, subinterval, numbers_to_skip, pointer));
        for (j = 1; j <= 3; j++) {
	...
	*/
        ...
        try {
            ...
            dataFile = "/ASCP2000.txt";
            if(null == dataFile) {
                System.out.println(format("OPening file from default location [%s].", filename));
                FileReader file = new FileReader(filename);
                buff = new BufferedReader(file);
            } else {
                System.out.println(format("Opening file from configured location [%s].", dataFile));
                InputStream is = getClass().getResourceAsStream(this.dataFile);
                if(null == is) {
                    throw new IllegalArgumentException("Unable to open datafile: "+dataFile+".");
                }
    ...
    public double[] getEphemerisCoefficients() {
        return ephemeris_coefficients;
    }
    public double[] getEphemerisDates() {
        return ephemeris_dates;
    }
}
